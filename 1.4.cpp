...
public:
    ...
    bool isConvex() {
        bool sign = getAbsVecProd(vertices[1] - vertices[0], vertices[2] - vertices[1]) > 0; // >= 0 ???
    ...
    double perimeter() const override { // fix!!!
        double res = 0;
...
public:
    ...
    bool isCongruentTo(const Shape& another) const override {
        ...
        for (int attempt = 1; attempt <= 2; ++attempt) {
            Polygon copy = *another_pol;
            if (attempt == 2)
                copy.reflect(Line(0, 0));
            auto direct = Vector(center() - copy.center(), false);
            for (size_t i = 0; i < n; ++i)
                copy.vertices[i] += direct;
            for (size_t i = 0; i < n * 2; ++i) {
                auto angle = getAngle(Vector(copy.vertices[0] - _center),
                                      Vector(vertices[i % n] - _center)) * 180 / M_PI;
                copy.rotate(_center, angle);
                if (copy == *this)
                    return true;
            }
        }
    ...
    bool isSimilarTo(const Shape& another) const override {
        ...
        for (size_t i = 0; i < n; ++i) {
            Polygon copy = *another_pol;
            copy.scale(Point(0, 0), (vertices[i] - vertices[(i + 1) % n]).abs() /
                                    (copy.vertices[i] - copy.vertices[(i + 1) % n]).abs());
            if (isCongruentTo(copy))
                return true;
        }
        return false;
    }
...
public:
    ...
    Circle ninePointsCircle() const {
        ...
        auto r = inscribedCircle().radius() / 2;
        return Circle(center, r);
    }
};
...
int main() {
    auto P1 = Polygon(Point(3, 4), Point(5, 6), Point(9, 5));
    auto P2 = Triangle(Point(1.5, 2), Point(2.5, 3), Point(4.5, 2.6));
    P2.rotate(Point(4, 9), 30);
    P2.reflect(Line(1, 1));
//    std::cout << P2.vertices;
    Shape& S1 = P1;
//    auto C = Circle(Point(0, 0), 5);
    auto E1 = Ellipse(Point(0, 0), Point(2, 0), 10);
    auto E2 = Ellipse(Point(0, -1), Point(0, 0), 5);
    std::cout << (E1.isSimilarTo(E2)) << '\n';
//    std::cout << (C.isCongruentTo(E)) << '\n';
//
}