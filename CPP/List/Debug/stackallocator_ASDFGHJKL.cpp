...
public:
    ...
    template <typename = std::enable_if_t<std::is_default_constructible<Allocator>::value>>
    List();
    ...
    List(size_t count, const T& value, const Allocator& alloc): List(alloc) {
    ...
    List(size_t count, const Allocator& alloc): List(alloc) {
        for (size_t i = 0; i < count; ++i) {
            try {
                emplace_push_back();
            } catch (...) {
...
int main() {
    auto lst = List<int, StackAllocator<int, 1000>>();
}