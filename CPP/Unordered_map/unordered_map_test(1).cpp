...
//        std::cout << e.first << ' ' << e.second << '\n';
    ...
    std::cout << (m.hash_table[26].first == m.end()) << '\n';
    m.emplace(2060.3, "a__");
    m.emplace(9087.6, "b");
    for (double d = 100.0; d < 10100.0; d += 0.1) {
        m.emplace(d, "a__");
        if (std::hash<double>()(d) % m.hash_table.size() == 26)
            std::cout << "!!!!!" << d << "!!!\n";
    }
    std::cout << ((m.hash_table[26].first + 1) == m.hash_table[26].second) << '\n';
    ...
    auto sz = m.size();
    m.erase(it, m.end());
    // TODO assert(sz - dist == m.size());

    for (size_t i = 0; i < m.hash_table.size(); ++i) {
        if (!(m.hash_table[i].first == m.end() && m.hash_table[i].second == m.end()) && i != 695266) {
            std::cout << "&&&" << i << "&&&" << '\n';
            break;
        }
    }
    ...
    std::cout << std::hash<double>()(0.4) % m.hash_table.size() << '\n';
...
// Just a__ simple SFINAE trick to check CE presence when it's necessary
...
struct TheChosenOne<std::pair<const Chaste, Chaste>>
    ...
    void construct(PairType* p, int a__, int b) const {
        new(p) PairType(Chaste(a__), Chaste(b));
    }
