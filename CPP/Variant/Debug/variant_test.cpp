...
void BasicTest() {
    ...
    static_assert(std::is_assignable<decltype(v), float>::value);
    static_assert(!std::is_assignable<decltype(v), std::vector<int>>::value);
...
//        Variant<int, const std::string> vvv = std::move(get<0>(v));
        auto temp = std::move(get<0>(v));
        Variant<int, const std::string> vvv = std::move(temp);
