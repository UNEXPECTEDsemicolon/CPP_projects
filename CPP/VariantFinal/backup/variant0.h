...
//                        create_head(std::forward<VarUnion>(source).head);
    ...
    public:
        ...
        /*explicit */Variant(const Variant& source): current(source.current), is_empty(source.current) {
            storage.copy_VarUnion(source.storage, source.current);
        }

        /*explicit */Variant(Variant&& source): current(source.current), is_empty(source.current) {
...
//        type_by_index_t<index, Types...>& emplace(T&& first_arg, Args&&... args) {
//            return emplace<index>(SpecialType(), std::forward<T>(first_arg), std::forward<Args>(args)...);
//        }
...
//        type_by_index_t<index, Types...>& emplace(const std::initializer_list<U>& il/*, Args&&... args*/) {
//            if (!is_empty) {
//                storage.destroy(current);
//                is_empty = true;
//            }
...
//        }
        ...
        T& emplace(const std::initializer_list<U>& il/*, Args&&... args*/) {
            return emplace<get_index_by_type_v<T, Types...>>(il/*, std::forward<Args>(args)...*/);
        }
