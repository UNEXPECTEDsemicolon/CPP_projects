...
int main() {
    ...
    static_assert(std::is_same<std::iterator_traits<List<int>::iterator>::iterator_category,
...
//    TestPerformance<std::list>();
    ...
    if (std::is_assignable<List<int>, std::list<int>>::value || std::is_assignable<std::list<int>, List<int>>::value) {
