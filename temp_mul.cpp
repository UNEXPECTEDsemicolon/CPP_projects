...
#define delete_all_in_root_to bn_delete(l); bn_delete(r); bn_delete(res); bn_delete(res_in_pow); bn_delete(diff); bn_delete(one);
...
// Инициализировать (начение BN десятичным представлением строки
...
//* !!! (not const) ex */ const char *bn_to_string(bn const *t, int radix);
...
int bn_init_string_radix(bn *t, char *init_string, int radix) {
    ...
    i++;
    for (; i<str_len; i++) {
        if (init_string[i] < '0' || init_string[i] > '9') return BN_NULL_OBJECT;
        if (bn_int_mul_to(t, radix)) return BN_NO_MEMORY;
        t->a[0] += (size_t)(init_string[i] - '0');
    }
...
int bn_init_string(bn *t, char *init_string) {
    return bn_init_string_radix(t, init_string, 10);
}

char *bn_to_string(bn *t, int radix) {
    ...
    if (t->n == 1 && t->a[0] == 0) {
        ...
        for (; bn_cmp(bn_for_div, zero) != 0; i++) {
            size_t mod = bn_int_div_to_and_get_mod(bn_for_div, radix);
            str[i] = '0' + (char) mod;
        }
        len = i + minus;
    }
...
static void pretty_print(bn *t) {
    char *s = bn_to_string(t, 10);
    printf("%s\n", s);
    free(s);
}
...
static int wrapped_bn_div_and_mod(bn *t, bn const *right, int for_mod) {
...
int bn_div_to(bn *t, bn const *right) {
    if (bn_is_OK(t) || bn_is_OK(right)) return BN_NULL_OBJECT;
    int code = wrapped_bn_div_and_mod(t, right, 0);
    if (code) return code;
    t->sign *= right->sign;
    if (t->sign < 0) {
        t->a[0] += 1;
...
int bn_mod_to(bn *t, bn const *right) {
    if (bn_is_OK(t) || bn_is_OK(right)) return BN_NULL_OBJECT;
    int code = wrapped_bn_div_and_mod(t, right, 1);
    if (code) return code;
...
int bn_root_to(bn *t, int n) {
    if (bn_is_OK(t) || t->sign < 0 && n % 2 == 0) return BN_NULL_OBJECT;
    if (n == 1)
        return BN_OK;
    int future_sign = t->sign;
    bn_abs(t);
    ...
    bn *res_in_pow = bn_new();
    if (bn_is_OK(res_in_pow)) {
        bn_delete(l);
        bn_delete(r);
        bn_delete(res);
        bn_delete(res_in_pow);
        return BN_NO_MEMORY;
    }
    bn *diff = bn_sub(r, l);
    if (bn_is_OK(diff)) {
    ...
    while (bn_cmp_abs(diff, one) > 0) {
        code = bn_add_to(res, r);
        if (code) {
            delete_all_in_root_to
            return code;
        }
        bn_int_div_to_and_get_mod(res, 2);
        code = bn_update(res_in_pow, res);
        if (code) {
            delete_all_in_root_to
            return code;
        }
        code = bn_pow_to(res_in_pow, n);
        if (code) {
            delete_all_in_root_to
            return code;
        }
        if (bn_cmp_abs(res_in_pow, t) < 0)
            code = bn_update(l, res);
        else if (bn_cmp_abs(res_in_pow, t) > 0)
            code = bn_update(r, res);
        else {
            code = bn_update(l, res);
            code = bn_update(r, res);
        }
        ...
        code = bn_update(diff, r);
        if (code) {
            delete_all_in_root_to
            return code;
        }
        code = bn_sub_to(diff, l);
        if (code) {
            delete_all_in_root_to
            return code;
        }
        code = bn_update(res, l);
        if (code) {
            delete_all_in_root_to
            return code;
        }
    }
    code = bn_update(res_in_pow, r);
    if (code) {
        delete_all_in_root_to
        return code;
    }
...
int main() {
    char *s1, *s2;
    s1 = (char *)malloc(10000ll * sizeof(char));
    s2 = (char *)malloc(10000ll * sizeof(char));
    char action;
    scanf("%s", s1);
    scanf("\n%c\n", &action);
    scanf("%s", s2);
    ...
    for (; s2[len2] != '\0'; len2++);
    len1++; len2++;
    s1 = (char *)realloc(s1, len1 * sizeof(char));
    s2 = (char *)realloc(s2, len2 * sizeof(char));
    bn *a = bn_new(), *b = bn_new();
    bn_init_string_radix(a, s1, 10);
    bn_init_string_radix(b, s2, 10);
    bn_mul_to(a, b);
    pretty_print(a);
    bn_delete(a);
    bn_delete(b);
